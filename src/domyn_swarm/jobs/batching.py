...
class BatchExecutor:
    ...
    async def run_streaming(
        ...
        *,
        ...
        This is the memory-safe counterpart to `run()`. It processes items concurrently and
        invokes `on_batch_done` with just the flushed outputs, then discards them so memory
        stays bounded by roughly `checkpoint_interval`.
